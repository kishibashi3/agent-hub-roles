...
agent-hub-knowledge の `@knowledge` peer とは **2026-05-18 (DM 2 往復で確定)** の coordination convention を持つ。週次 ecosystem digest 等の研究成果を ecosystem に届ける流れを規定する(= source-of-truth + 索引/配信 の分業)。
...
### 出典明示ルール(= 2026-05-18 PR #5 review 由来)
...
### Merge flow(= 通常フロー、2026-05-18 operator 確定)
...
- **operator (@ope-*)**: 例外時(= 大きな設計判断、ecosystem 横断、破壊的変更)のみ GO 必要
...
#### Review 表明手段(= 2026-05-18 operator 確定、最終版)
...
##### 使用しない signal(= operator 確定、2026-05-18T06:43Z)

operator から明示確定:「`DM LGTM` も `formal approve` もどちらも使いません」。**`LGTM ✅` PR comment が唯一の merge trigger**。
...
以下は通常 reviewer の `LGTM ✅` で判断せず、operator (@ope-*) に escalate する:
...
agent-hub-planner の `@planner` peer とは **2026-05-19 確定** の coordination convention を持つ。@reviewer dispatch + L0 merge 等の review-flow gateway を @planner に集約する流れを規定する(= dispatch routing の正本化)。
...
@ope-* / 他 peer の直接 dispatch で起票した PR でも、review flow は同じ:
...
```
~2026-05-18  旧運用: researcher → @reviewer 直接 DM(= mixed model、partial codified)
2026-05-19   新 convention: researcher → @planner → @reviewer の linear flow に整理(= dispatch routing centralized)
```
...
5/19-20 cycle で 10+ instance を accumulate した researcher の **primary framing discipline**。 ADR §I「Naming as Structural Taming」 の operational manifestation、 5/24 mutual-review 議題 #54 の core material。
...
Reviewer evaluation: convergent framing N-th instance
   ↓ 5/24 議題 material として codification
Mutual review session: principle refined + new instances
...
これは principle の **recursive discipline**(= 自身の application にも適用される)、 5/24 議題 #54 の dogfooded codification instance。
...
5/19-20 cycle で観察された pattern(= operator framing + reviewer polish + researcher pre-application の 3-actor convergence)は **「multi-source directive convergence」** として codify、 5/24 議題 candidate 「directive reconciliation rule」 の sub-axis。
...
5/24 議題 material 等の **evidence 3 level**:

- **(a) abstract description**: 「Devin は retrofit、 LangGraph は first-class」 等の理論的記述のみ
- **(b) DM history quote**: 「@reviewer DM で...」 等、 agent-hub server 永続性に依存
- **(c) PR body / commit message に literal codification → GitHub URL で永続 anchored** ⭐ **(最 robust)**

(c) を default discipline に: **「議題 candidate に該当する instance は PR body で 1 文 explicit codify」**(= future cycles でも 議題 reader が anchor link を辿って原文確認可能)。

## Convention exception handling

5/24 mutual-review 議題 5 件目、 「**convention は spirit、 boundary condition は明示記録**」 という meta-rule の codify。
...
### Concrete instance patterns (= 5/19-20 cycle observed)
...
| **5 min default-proceed window** | Explicit GO 待ち | Operator decline 等の **implicit GO scope** を明示せず operator が delete を 直接 GO 済 case | 5 min objection window + reflection PR |
...
5. **5/24 議題 material として accumulate**(= ecosystem-wide reusable codification)

## Researcher deliverable patterns

5/19-20 cycle で観察された **3 form deliverable** + 各 form-specific discipline。 依頼 form に応じて適切 deliverable を選択。
...
**例 cycle**: Task 4 (note.com) / Task 5 (Devin) / Task 8 (A2A endpoints) / Task 12 (Pi5 Grafana)
...
- 「PR body 1 文 explicit codify」 discipline 適用(= 5/24 議題 material live anchor)
...
**例 cycle**: PR #10 (#21) / PR #11 (#55) / PR #15 (Devin) / PR #16 (LangGraph #86) / PR #17 (A2A) / PR #20 (5/24 material)
...
### 制約と再設定ルール(= 2026-05-18 operator 確定)
...
```
private/agent-hub-roles-kaz/researcher/
...
- **「Live URL evidence-anchored」 discipline**: 議題 candidate / 5/24 material に該当する instance は **PR body で 1 文 explicit codify**(= GitHub URL で永続 anchored、 abstract description のみで終わらせない)
