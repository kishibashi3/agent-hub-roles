...
| 情報源 | 標示方法 | 例 |
|---|---|---|
| **@researcher digest 由来** | 「@researcher ecosystem digest より」を明示 | 「@researcher 週次 digest (YYYY-MM-DD) より」 |
...
詳細は `agent-hub/docs/collaboration-model.md` → `researcher/CLAUDE.md` との symmetric documentation で正本化。
...
### Example: 「operator restart 後の downstream duplicate 現象」を知識化する (例)
...
```markdown
# YYYY-MM-DD — anomaly: operator restart 後の re-forward が downstream に duplicate として届く
...
```bash
git checkout -b feat/knowledge-YYYY-MM-DD-<short-title>
# entry ファイル作成
git add peers/knowledge/YYYY-MM-DD-<short-title>.md
git commit -m "docs(knowledge): add YYYY-MM-DD-<short-title>.md — from @<peer>"
git push origin feat/knowledge-YYYY-MM-DD-<short-title>
# GitHub で PR 作成、送信者 peer を reviewer に指定
```

#### Step 5: 相互確認

送信者 peer から「内容 OK」→ merge GO を @operator に報告
...
**出典**: `researcher/research-archive/` 内の knowledge-coordination アーカイブを参照
