...
> このファイル は [agent-hub-roles](https://github.com/kishibashi3/agent-hub-roles) の **operator template** です。 fork した workspace の root から `claude` を 起動 すると、 ここ が ecosystem セットアップ の 出発点 として ロード されます。 自由 に 編集 して 構いません (= 上流 `private/operation/CLAUDE.md` を 参考 に 書き起こした、 ecosystem 共通 規約 は repo root の `../CLAUDE.md` 参照)。

> ⚠️ operator は **bridge worker ではない**。 人間 が 使う Claude Code セッション の persona です (Q-new-1 確認済、 2026-05-19)。 他 4 role (reviewer / planner / researcher / writer) と 異なり、 別 process の `agent-hub-bridge-claude` は 立ち上げず、 Claude Code セッション 自身 が agent-hub-plugin 経由 で `@operator` として register する 想定。
>
> 2026-05-19 operator 指示: roles 命名規則 統一 のため handle は `@operator` (= 個別 host-suffix なし)。 `start.sh` (M1) では 「operator 指定 / 引数なし」 で この directory への `claude` 起動 を 案内 する。
...
- **agent-hub での handle**: `@operator` (= roles の 命名規則 (reviewer / planner / researcher / writer / operator) に 合わせて 統一、 2026-05-19 operator 指示)
...
| package | 何用 |
|---|---|
| `agent-hub-plugin` (= [kishibashi3-plugins-claude](https://github.com/kishibashi3/kishibashi3-plugins-claude)) | この Claude Code セッション を agent-hub に 接続 (= @operator として register) |
| `agent-hub-bridges[claude]` (= [agent-hub-bridges](https://github.com/kishibashi3/agent-hub-bridges)) | 他 4 role (reviewer / planner / researcher / writer) を bridge worker として spawn する コマンド |
...
|---|---|---|---|---|
| @reviewer | kaz | <fork>/reviewer/ | 12345 | 2026-05-19T22:00Z |
| @planner | kaz | <fork>/planner/ | 12346 | 2026-05-19T22:05Z |

## Activity log

- 2026-05-19T22:00Z: spawn @reviewer (pid 12345)
- 2026-05-19T22:05Z: spawn @planner (pid 12346)
```
...
## secure_mode (= 2026-05-19 確認 済 ecosystem 規約)
...
- **handle** (`@operator`) は 統一 命名 で 固定 (= 個別 suffix は つけない、 2026-05-19 operator 指示)
...
- [agent-hub](https://github.com/kishibashi3/agent-hub) — server
- [agent-hub-bridges](https://github.com/kishibashi3/agent-hub-bridges) — engine 層 monorepo (= `agent-hub-bridge-claude` 等)
